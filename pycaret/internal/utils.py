...
def create_markdown_report(model, report_name,model_type):
    ...
    report_model_type = model_type+report_type
    report_name = helper.get_report_name(report_model_type)

    # Set the report title - align in middle #TODO
    mdFile.new_header(level=1, title=report_name + ' Report', style='setext')
    ...
    for plot_type in model_list:
        ...
        mdFile.new_line(mdFile.new_inline_image(text=plot_name, path='./' + image_name) + '.png')
