import sys

from pycaret.utils._show_versions import show_versions

version_ = "3.3.2"

__version__ = version_

__all__ = ["show_versions", "__version__"]

# Pycaret supports python 3.9 through 3.14
# This code is to avoid issues with unsupported Python versions
# example (see package versions): https://github.com/pycaret/pycaret/issues/3746

if sys.version_info < (3, 9):
    raise RuntimeError(
        "Pycaret only supports Python 3.9 through 3.14. Your actual Python version: ",
        sys.version_info,
        "Please UPGRADE your Python version.",
    )
elif sys.version_info >= (3, 15):
    raise RuntimeError(
        "Pycaret only supports Python 3.9 through 3.14. Your actual Python version: ",
        sys.version_info,
        "Please DOWNGRADE your Python version.",
    )
