...
class DashboardLogger:
    ...
    def log_model(
        ...
        from pycaret.internal.tabular import get_pipeline_estimator_label, plot_model, pull, _get_model_name, exp_name_log, predict_model, _is_unsupervised
        console = get_logger()
    ...
    def log_experiment(self, log_profile, profile_kwargs, log_data, ml_usecase, functions, experiment_custom_tags, runtime, display):
        ...
        [logger.log_artifact("Transformation Pipeline.pkl", "transformation_pipe") for logger in self.loggers]
        ...
        if log_profile:
            ...
            [logger.log_artifact("Data Profile.html", "data_profile") for logger in self.loggers]
        ...
        if log_data:
            if not _is_unsupervised(ml_usecase):
                ...
                [logger.log_artifact("Test.csv", "test_data") for logger in self.loggers]
                os.remove("Train.csv")
                os.remove("Test.csv")
            else:
