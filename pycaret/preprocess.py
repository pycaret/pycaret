  ...
  """
    ...
    def fit(self, dataset, y=None):  #
        ...
        if self.numeric_strategy == "mean":
            self.numeric_stats = (
                data.drop(self.target, axis=1).select_dtypes(include=["float64", "int64"]).apply(np.nanmean)
            )
        else:
            self.numeric_stats = (
                data.drop(self.target, axis=1).select_dtypes(include=["float64", "int64"]).apply(np.nanmedian)
            )
  ...
  """
    ...
    def fit(self, dataset, y=None):
        ...
        self.numeric_features = (
            data.drop(self.target, axis=1, errors="ignore").select_dtypes(include=["float64", "int64"]).columns
        )
  ...
  """
    ...
    def transform(self, dataset, y=None):
        ...
        if "multiply" in self.interactions:
            ...
            data_multiply = pd.concat(
                [
                    data.drop(self.target, axis=1, errors="ignore").mul(col[1], axis="index")
                    for col in data.drop(self.target, axis=1, errors="ignore").iteritems()
                ],
                axis=1,
            )
        ...
        if "divide" in self.interactions:
            ...
            data_divide = pd.concat(
                [
                    data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].div(col[1], axis="index")
                    for col in data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].iteritems()
                ],
                axis=1,
            )
            data_divide.columns = [
                "_divide_".join([i, j])
                for j in data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].columns
                for i in data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].columns
            ]
        ...
        if "add" in self.interactions:
            ...
            data_add = pd.concat(
                [
                    data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].add(col[1], axis="index")
                    for col in data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].iteritems()
                ],
                axis=1,
            )
        ...
        if "subtract" in self.interactions:
            ...
            data_substract = pd.concat(
                [
                    data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].sub(col[1], axis="index")
                    for col in data.drop(self.target, axis=1, errors="ignore")[self.numeric_columns].iteritems()
                ],
                axis=1,
            )
    ...
    def fit_transform(self, dataset, y=None):
        ...
        self.ohe_columns = [
            i
            for i in data.columns
            if len(data[i].unique()) == 2 and data[i].unique()[0] in [0, 1] and data[i].unique()[1] in [0, 1]
        ]
        ...
        if "divide" in self.interactions:
            data_divide = pd.concat(
                [
                    data.drop(self.target, axis=1)[self.numeric_columns].div(col[1], axis="index")
                    for col in data.drop(self.target, axis=1)[self.numeric_columns].iteritems()
                ],
                axis=1,
            )
            data_divide.columns = [
        ...
        if "add" in self.interactions:
            data_add = pd.concat(
                [
                    data.drop(self.target, axis=1)[self.numeric_columns].add(col[1], axis="index")
                    for col in data.drop(self.target, axis=1)[self.numeric_columns].iteritems()
                ],
                axis=1,
            )
        ...
        if "subtract" in self.interactions:
            data_substract = pd.concat(
                [
                    data.drop(self.target, axis=1)[self.numeric_columns].sub(col[1], axis="index")
                    for col in data.drop(self.target, axis=1)[self.numeric_columns].iteritems()
                ],
                axis=1,
            )
...
class Reduce_Dimensions_For_Supervised_Path(BaseEstimator, TransformerMixin):
  ...
  """
    ...
    def __init__(self, target, method="pca_liner", variance_retained_or_number_of_components=0.99, random_state=42):
        self.target = target
