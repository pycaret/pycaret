...
def setup(
    ...
    global X, y, X_train, X_test, y_train, y_test, seed, prep_pipe, experiment__, folds_shuffle_param, n_jobs_param, create_model_container, master_model_container, display_container, exp_name_log, logging_param, log_plots_param, fix_imbalance_param, fix_imbalance_method_param
    ...
    if sampling is True and data.shape[0] > 25000:  # change this back to 25000
        ...
        for i in split_perc:
            ...
            X_, X__, y_, y__ = train_test_split(
                X, y, test_size=1 - i, stratify=y, random_state=seed, shuffle=data_split_shuffle
            )
            X_train, X_test, y_train, y_test = train_test_split(
                X_, y_, test_size=0.3, stratify=y_, random_state=seed, shuffle=data_split_shuffle
            )
        ...
        if sample_size == "" or sample_size == "1":
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=1 - train_size, stratify=y, random_state=seed, shuffle=data_split_shuffle
            )
            ...
            X_selected, X_discard, y_selected, y_discard = train_test_split(
                X, y, test_size=1 - sample_n, stratify=y, random_state=seed, shuffle=data_split_shuffle
            )
        ...
        functions = pd.DataFrame(
            [
                ...
                [
                    "Sampled Data",
                    "(" + str(X_train.shape[0] + X_test.shape[0]) + ", " + str(data_before_preprocess.shape[1]) + ")",
                ],
                ["Transformed Train Set", X_train.shape],
                ["Transformed Test Set", X_test.shape],
                ["Numeric Imputer ", numeric_imputation],
    ...
    **kwargs: 
    ...
    if estimator == "lr":
        ...
        model = CatBoostClassifier(
            random_state=seed, silent=True, thread_count=n_jobs_param, **kwargs
    ...
    model_results = model_results.style.apply(
        lambda x: ["background: yellow" if (x.name == "Mean") else "" for i in x], axis=1
    )
...
def ensemble_model(
    ...
    if method == "Bagging":
        ...
        model = BaggingClassifier(
            model, bootstrap=True, n_estimators=n_estimators, random_state=seed, n_jobs=n_jobs_param
        )
    ...
    model_results_unpivot = pd.melt(
        model_results, value_vars=["Accuracy", "AUC", "Recall", "Prec.", "F1", "Kappa", "MCC"]
    )
...
def plot_model(
    estimator, plot="auc", save=False, verbose=True, system=True  # added in pycaret 2.0.0  # added in pycaret 2.0.0
):  # added in pycaret 2.0.0
    ...
    if plot == "auc":
        ...
        viz = ValidationCurve(model, param_name=param_name, param_range=param_range, cv=10, random_state=seed)
        logger.info("Fitting Model")
...
def compare_models(
    ...
    logger.info(
        """compare_models(blacklist={}, whitelist={}, fold={}, round={}, sort={}, n_select={}, turbo={}, verbose={})""".format(
            str(blacklist), str(whitelist), str(fold), str(round), str(sort), str(n_select), str(turbo), str(verbose)
        )
    )
    ...
    progress = ipw.IntProgress(
        value=0, min=0, max=(fold * len_mod) + opt + n_select_num, step=1, description="Processing: "
    )
    ...
    lr = LogisticRegression(
        random_state=seed
    )  # dont add n_jobs_param here. It slows doesn Logistic Regression somehow.
    ...
    if y.value_counts().count() > 2:
        ...
        compare_models_ = master_display.style.apply(
            highlight_max, subset=["Accuracy", "Recall", "Prec.", "F1", "Kappa", "MCC"]
        ).applymap(highlight_cols, subset=["TT (Sec)"])
    else:

        compare_models_ = master_display.style.apply(
            highlight_max, subset=["Accuracy", "AUC", "Recall", "Prec.", "F1", "Kappa", "MCC"]
        ).applymap(highlight_cols, subset=["TT (Sec)"])
...
def tune_model(
    ...
    if estimator == "knn":
        ...
        model_grid = RandomizedSearchCV(
            estimator=KNeighborsClassifier(n_jobs=n_jobs_param),
        ...
        if custom_grid is not None:
            ...
            param_grid = {"C": np.arange(0, 10, 0.001), "penalty": ["l1", "l2"], "class_weight": ["balanced", None]}
        model_grid = RandomizedSearchCV(
            estimator=LogisticRegression(random_state=seed, n_jobs=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=GaussianProcessClassifier(random_state=seed, n_jobs=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        if custom_grid is not None:
            ...
            param_grid = {"C": np.arange(0, 50, 0.01), "class_weight": ["balanced", None]}

        model_grid = RandomizedSearchCV(
            estimator=SVC(gamma="auto", C=1, probability=True, kernel="rbf", random_state=seed),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=SGDClassifier(loss="hinge", random_state=seed, n_jobs=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        if custom_grid is not None:
            ...
            param_grid = {"alpha": np.arange(0, 1, 0.001), "fit_intercept": [True, False], "normalize": [True, False]}
        ...
        model_grid = RandomizedSearchCV(
            estimator=RandomForestClassifier(random_state=seed, n_jobs=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=AdaBoostClassifier(base_estimator=base_estimator_input, random_state=seed),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=ExtraTreesClassifier(random_state=seed, n_jobs=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=XGBClassifier(random_state=seed, n_jobs=n_jobs_param, verbosity=0),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=lgb.LGBMClassifier(random_state=seed, n_jobs=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=CatBoostClassifier(random_state=seed, silent=True, thread_count=n_jobs_param),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
            cv=cv,
        ...
        model_grid = RandomizedSearchCV(
            estimator=BaggingClassifier(
                base_estimator=_estimator_.base_estimator, random_state=seed, n_jobs=n_jobs_param
            ),
            param_distributions=param_grid,
            scoring=optimize,
            n_iter=n_iter,
...
def stack_models(
    ...
    logger.info(
        """stack_models(estimator_list={}, meta_model={}, fold={}, round={}, method={}, restack={}, plot={}, choose_better={}, optimize={}, finalize={}, verbose={})""".format(
            str(estimator_list),
            str(meta_model),
            str(fold),
            str(round),
    ...
    if plot:
        ...
        ax = sns.heatmap(
            base_prediction_cor, vmin=0.2, vmax=1, center=0, cmap="magma", square=True, annot=True, linewidths=1
        )
...
def create_stacknet(
    ...
    logger.info(
        """create_stacknet(estimator_list={}, meta_model={}, fold={}, round={}, method={}, restack={}, choose_better={}, optimize={}, finalize={}, verbose={})""".format(
            str(estimator_list),
            str(meta_model),
            str(fold),
            str(round),
...
def interpret_model(estimator, plot="summary", feature=None, observation=None):
    ...
    if plot == "summary":
        ...
        if feature == None:
            ...
            logger.warning(
                "No feature passed. Default value of feature used for correlation plot: " + str(X_test.columns[0])
            )
        ...
        if model_name in type1:
            ...
            if observation is None:
                ...
                if model_name == "LGBMClassifier":
                    ...
                    return shap.force_plot(
                        explainer.expected_value[1], shap_values[0][row_to_show], data_for_prediction
                    )
            ...
            if observation is None:
                ...
                logger.info("interpret_model() succesfully completed......................................")
                return shap.force_plot(
                    explainer.expected_value, shap_values[row_to_show, :], X_test.iloc[row_to_show, :]
                )
...
def evaluate_model(estimator):
    ...
    d = interact(
        plot_model, estimator=fixed(estimator), plot=a, save=fixed(False), verbose=fixed(True), system=fixed(True)
    )
...
def predict_model(
    estimator, data=None, probability_threshold=None, platform=None, authentication=None, verbose=True
):  # added in pycaret==2.0.0
    ...
    if type(estimator) is str:
        if platform == "aws":
            estimator_ = load_model(
                str(estimator), platform="aws", authentication={"bucket": authentication.get("bucket")}, verbose=False
            )
...
def optimize_threshold(estimator, true_positive=0, true_negative=0, false_positive=0, false_negative=0):
    ...
    logger.info(
        """optimize_threshold(estimator={}, true_positive={}, true_negative={}, false_positive={}, false_negative={})""".format(
            str(estimator), str(true_positive), str(true_negative), str(false_positive), str(false_negative)
        )
    )
...
def automl(optimize="Accuracy", use_holdout=False):
    ...
    logger.info("""automl(optimize={}, use_holdout={})""".format(str(optimize), str(use_holdout)))
...
def models(type=None):
    ...
    df = pd.DataFrame({"ID": model_id, "Name": model_name, "Reference": model_ref, "Turbo": model_turbo})
