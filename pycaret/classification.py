...
def compare_models(exclude = None, #changed blacklist to exclude in pycaret==2.1
    ...
    def model_name(model_abbrev):
        for key,value in model_dict.items():
            if value == model_abbrev:
                return key
            else:
                return '??'         

    def model_abbrev(model_name):
    ...
    model_names = ['Logistic Regression',
                   ...
                   'CatBoost Classifier']          
    model_names = [i + ' ('+model_abbrev(i)+')' for i in model_names]
    ...
    for i in sorted_model_names:
        ...
        if verbose:
            if html_param:
                update_display(monitor, display_id = 'monitor')
        progress.value += 1
        model_short = i[:i.find('(')-1] #strip the abbreviation off the end
        k = model_dict.get(model_short)
