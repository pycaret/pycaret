...
def setup(
    ...
    if hasattr(data, "shape"):
        if target is None:
            sys.exit(
                "(Type Error): When DataFrame is passed as data param. Target column containing text must be specified in target param."
            )
...
def create_model(
    model=None, multi_core=False, num_topics=None, verbose=True, system=True, **kwargs  # added in pycaret==2.0.0
):  # added in pycaret==2.0.0
...
def plot_model(
    model=None, plot="frequency", topic_num=None, save=False, system=True  # added in pycaret 2.0.0
):  # added in pycaret 2.0.0):
...
def save_model(model, model_name, verbose=True):  # added in pycaret==2.0.0
...
def load_model(model_name, verbose=True):  # added in pycaret==2.0.0
